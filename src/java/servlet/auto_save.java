 ...
 * @author w15rdknl
 */
public class auto_save extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            ps.setString(1, p_order_memo_area);
            ps.setString(2, p_ftp_number);

            try {
                ps.executeUpdate();
            } catch (Exception e) {
                order_update_flag = "false";  // error
                System.out.println("modify order memo here: " + e.getMessage());
            } finally {
                try {
                    ps.close();
                    connection.close();
