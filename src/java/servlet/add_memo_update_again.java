 ...
 * @author w15rdknl
 */
public class add_memo_update_again extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            sql_stmt = "IF NOT EXISTS (SELECT * FROM MEMO_UPDATE"
                    + " WHERE ftp_number = " + p_directory
                    + " AND file_name = " + "\'" + p_filename + "\'"
                    + " )"
                    + " INSERT INTO MEMO_UPDATE (ftp_number, file_name, memo) VALUES ("
                    + p_directory + ","
                    + "\'" + p_filename + "\'" + ","
                    + "\'" + p_memo + "\'"
                    + ")"
                    + " ELSE UPDATE MEMO_UPDATE SET memo ="
                    + "\'" + p_memo + "\'"
                    + " WHERE ftp_number = " + p_directory
                    + " AND file_name = " + "\'" + p_filename + "\'";
            ...
            try {
                ...
                memo_add_flag = "false";
                System.out.println("error adding memo here: " + e.getMessage());
            } finally {
                try {
                    if (ps != null) {
                        ps.close();
