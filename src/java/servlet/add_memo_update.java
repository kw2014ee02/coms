 ...
 * @author w15rdknl
 */
public class add_memo_update extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            //SQL
            int numberOfRows = 0;
            PreparedStatement ps = connection.prepareStatement("SELECT COUNT(*) FROM [winpak].[dbo].[MEMO] WHERE DIRECTORY = ? AND FILENAME = ?");
            ...
            // SQL
            if (numberOfRows == 0) {     // 添加新行           

                //得到表的行数
                ps = connection.prepareStatement("SELECT COUNT(*) FROM [winpak].[dbo].[MEMO]");
                ResultSet rs = ps.executeQuery();
                if (rs.next()) {
                    numberOfRows = rs.getInt(1);
                    System.out.println("numberOfRows= " + numberOfRows);
                ...
                String sql_stmt = "UPDATE [winpak].[dbo].[MEMO]"
                        + " SET MEMO = ?"
                        + " WHERE DIRECTORY = ? AND FILENAME = ?";

                ps = connection.prepareStatement(sql_stmt);

                ps.setString(1, p_memo);
                ps.setString(2, p_directory);
                ps.setString(3, p_filename);
                ...
                try {
                    ...
                    memo_add_flag = "false";  // error
                    System.out.println("modify file memo here: " + e.getMessage());
                } finally {
                    try {
                        ps.close();
                        connection.close();
